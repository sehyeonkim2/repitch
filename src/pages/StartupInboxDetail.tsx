...
const StartupInboxDetail = () => {
  const { id } = useParams<{ id: string }>();
  const navigate = useNavigate();
  const { getStartupProposal, createStartupChatRoom } = useApp();
  const [decision, setDecision] = useState<Decision | null>(null);
  const [chatRoomId, setChatRoomId] = useState<string | null>(null);

  const proposal = id ? getStartupProposal(id) : null;

  if (!proposal) {
    return (
      <div className="flex flex-col min-h-full">
        <MobileHeader title="받은 제안" back={() => navigate("/startup/inbox")} view="startup" />
        <main className="flex-1 flex flex-col items-center justify-center text-center px-6 py-12">
          ...
          <Button variant="primary" icon="restart_alt" onClick={() => navigate("/startup/inbox")}>
            수령함으로 돌아가기
          </Button>
        </main>
      </div>
  ...
  const handleAccept = () => {
    const roomId = createStartupChatRoom(
      proposal.id,
      inf,
    ...
    <div className="flex flex-col min-h-full bg-surface-container-low">
      ...
      <main className="flex-1 px-4 py-4 pb-24 space-y-3">
        ...
        <CollapsibleSection title="제안 조건" icon="local_offer" defaultOpen>
          <div className="space-y-2 text-caption">
            ...
            <div className="flex justify-between">
              <span className="text-on-surface-variant">타겟 소구점</span>
      ...
      <StickyAction>
        <div className="grid grid-cols-3 gap-2">
          ...
          <Button variant="primary" fullWidth icon="check_circle" onClick={handleAccept}>
            수락
          </Button>
        </div>
      </StickyAction>
          ...
          >
            ...
            >
              ...
              <p className="font-body-md text-body-md text-on-surface-variant mb-md">
                @{inf.handle}님과 채팅방이 생성되었습니다. Startup Chat에서 협업을 이어가세요.
              </p>
              ...
              <div className="grid grid-cols-2 gap-2">
                <Button variant="ghost" fullWidth onClick={() => navigate("/startup/inbox")}>
                  닫기
                </Button>
                <Button
                  variant="primary"
                  fullWidth
                  icon="chat_bubble"
                  onClick={() => chatRoomId && navigate(`/startup/chat/${chatRoomId}`)}
                >
                  Chat 시작
                </Button>
              </div>
        ...
        {decision === "negotiate" && (
          ...
          >
            ...
            >
              ...
              <p className="font-body-md text-body-md text-on-surface-variant mb-md">
                채팅방이 생성되었습니다. Startup Chat에서 인플루언서와 협의를 이어가세요.
              </p>
              <div className="grid grid-cols-2 gap-2">
                <Button variant="ghost" fullWidth onClick={() => navigate("/startup/inbox")}>
                  닫기
...
export default StartupInboxDetail;

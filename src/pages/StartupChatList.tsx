...
const StartupChatList = () => {
  ...
  const { startupChatRooms } = useApp();
  const rooms = Object.values(startupChatRooms).sort((a, b) =>
        ...
        {rooms.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20 text-center">
            ...
            <p className="text-caption text-on-surface-variant mt-1 max-w-[240px]">
              인플루언서 제안을 수락하거나 협의하면 채팅방이 자동으로 생성됩니다
            </p>
          </div>
        ) : (
            ...
            {rooms.map((room, i) => {
              ...
              return (
                <motion.button
                  ...
                  type="button"
                  onClick={() => navigate(`/startup/chat/${room.id}`)}
